...
from app.rag.langgraph_nodes import (analyze_query_node,
                                     ...
                                     create_retrieve_node, generate_node,
                                     validate_node)
...
def route_after_validation(state: AgentState) -> str:
    ...
    if needs_refinement:
        ...
        return (
            "refine"  # Future: could route back to analyze_query with refined question
        )
    else:
...
def build_advanced_rag_graph(mongo_service: MongoVectorService) -> Any:
    ...
    workflow.add_node("analyze_query", analyze_query_node)
    workflow.add_node(
        "exact_match_retrieve", create_exact_match_retrieve_node(mongo_service)
    )
    workflow.add_node("ranking_retrieve", create_ranking_retrieve_node(mongo_service))
    workflow.add_node(
        "aggregation_retrieve", create_aggregation_retrieve_node(mongo_service)
    )
