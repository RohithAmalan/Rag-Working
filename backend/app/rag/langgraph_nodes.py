...
async def analyze_query_node(state: AgentState) -> dict[str, Any]:
    ...
    requires_ranking = any(
        token in q_lower
        for token in ["top", "highest", "lowest", "maximum", "minimum", "max", "min"]
    ) and any(
        token in q_lower
        for token in [
            "price",
    ...
    requires_exact_match = (
        bool(entities["ids"]) or bool(entities["phones"]) or bool(entities["emails"])
    )
    ...
    if requires_exact_match:
        ...
        token in q_lower
        for token in ["all", "list all", "count", "total", "sum", "average"]
    ):
    ...
    logger.info(
        f"Query analysis: intent={query_intent}, ranking={requires_ranking}, exact={requires_exact_match}"
    )
...
async def retrieve_node(
    ...
    try:
        ...
        for query in queries:
            logger.info(
                f"Retrieving {candidate_count_per_query} candidates for: {query[:50]}..."
            )
            ...
            has_strong_primary_match = any(
                float(chunk.get("similarity_score", 0.0)) >= 0.95 for chunk in chunks
            )
        ...
        reranked_chunks = reranker.rerank(
            query=question, chunks=all_chunks, top_k=top_k
        )
...
async def generate_node(state: AgentState) -> dict[str, Any]:
    ...
    try:
        ...
        result = citation_gen.generate_with_citations(
            question=question, chunks=formatted_chunks, source_types=source_types
        )
...
async def aggregation_retrieve_node(
    ...
    try:
        ...
        logger.info(
            f"Aggregation retrieval found {len(chunks)} chunks (extended from {top_k} to {extended_top_k})"
        )
...
async def validate_node(state: AgentState) -> dict[str, Any]:
    ...
    needs_refinement = (
        confidence < LOW_CONFIDENCE_THRESHOLD or has_uncertainty
...
def create_parallel_retrieve_node(mongo_service: MongoVectorService):
    ...
    async def parallel_retrieve_node(state: AgentState) -> dict[str, Any]:
        ...
        logger.info(
            f"Parallel retrieval from {len(files_to_search)} sources: {files_to_search}"
        )

        # Parallel retrieval tasks
        async def retrieve_from_file(
            file_name: str | None,
            ...
            try:
                ...
                logger.info(
                    f"Retrieved {len(chunks)} chunks from {file_name or 'all files'}"
                )
        ...
        logger.info(
            f"Parallel retrieval complete: {total_chunks} total chunks from {len(results)} sources"
        )
...
async def aggregate_results_node(state: AgentState) -> dict[str, Any]:
    ...
    if aggregation_type == "compare":
        ...
        logger.info(
            f"Comparison aggregation: {len(merged_results)} total chunks from {len(parallel_chunks)} files"
        )
        ...
        logger.info(
            f"Ranking aggregation: {len(merged_results)} unique chunks after deduplication"
        )
    ...
    return {
        ...
        "comparison_results": (
            comparison_results if aggregation_type == "compare" else None
        ),
...
async def generate_comparison_node(state: AgentState) -> dict[str, Any]:
    ...
    if comparison_results:
        ...
        context = "\n\n".join(
            [chunk.get("content", "") for chunk in merged_results[:10]]
        )
