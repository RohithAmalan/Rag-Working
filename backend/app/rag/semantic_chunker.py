...
class SemanticChunker:
    ...
    def chunk_text(
        ...
        boundaries = self._find_boundaries(similarities, threshold=similarity_threshold)
    ...
    def _find_boundaries(
        self, similarities: List[float], threshold: float = 0.7
    ) -> List[int]:
    ...
    def _create_chunks(
        ...
        for i in range(len(boundaries) - 1):
            ...
            if len(chunk_text) > max_chunk_size:
                ...
                sub_chunks = self._split_large_chunk(chunk_sentences, max_chunk_size)
    ...
    def _split_large_chunk(self, sentences: List[str], max_size: int) -> List[Dict]:
    ...
    def chunk_document(
        self, document: str, metadata: Dict = None, **kwargs
    ) -> List[Dict]:
        """
        ...
        if metadata:
            for i, chunk in enumerate(chunks):
                chunk.update(
                    {"chunk_index": i, "total_chunks": len(chunks), **metadata}
                )
