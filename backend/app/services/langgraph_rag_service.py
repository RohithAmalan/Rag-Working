...
from app.rag.langgraph_pipeline import (
...
class LangGraphRAGService:
    ...
    def __init__(
        ...
        if workflow_mode == "multi_agent":
            ...
            logger.info("LangGraph RAG service initialized with Phase 3 multi-agent workflow")
        elif workflow_mode == "advanced":
            self.workflow = build_advanced_rag_graph(mongo_service)
            logger.info("LangGraph RAG service initialized with Phase 2 advanced routing")
        else:
            self.workflow = build_basic_rag_graph(mongo_service)
            logger.info("LangGraph RAG service initialized with Phase 1 linear workflow")
    ...
    async def query(
        ...
        """
        logger.info(f"LangGraph query: {question[:100]}... (file={selected_file}, top_k={top_k})")
